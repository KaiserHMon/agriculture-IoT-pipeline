...
def get_data_date(file_path, date_columns):
    ...
    try:
        ...
        logger.warning(f"Could not extract date from data in {file_path}: {e}. Using file date.")
...
def run_ingestion():
    ...
    bronze_prefix = s3_conf['bronze_prefix']
    ...
    # 3. Scan for files
    allowed_ext = tuple(ingest_conf['allowed_extensions'])
    ...
    # 4. Upload and Move each file
    for filename in files_to_upload:
        local_path = os.path.join(input_dir, filename)
        
        # Determine the date for partitioning
        record_date = get_data_date(local_path, ingest_conf['date_columns'])
